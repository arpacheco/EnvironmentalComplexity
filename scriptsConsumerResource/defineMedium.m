...
for i = 1:length(modelNames)
    ...
    for j = 1:length(medRxns)
        ...
        else % if a reaction already had a lower bound, set it to that (unless it's O2, which should be constrained more)
            if strcmp(medRxns(j),'EX_o2(e)')
                model.lb(medRxns(j)) = -1;
            else
